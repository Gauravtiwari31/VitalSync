...
export default function DoctorBillingPage({searchParams:{hospitalKey,hospitalName}}:any) {
    ...
    const fetchDoctorAndPatientData = async () => {
      try {
        ...
        if (doctorResult.status === "fulfilled" && doctorResult.value.success) {
            const allDoctorData=doctorResult.value.doctorData;
            // console.log(allDoctorData);
            if(hospitalKey){
              const relevantDepts = allDoctorData.allDept.filter(
                (dept: any) => dept?.dept?.hospital?.id === hospitalKey
              );

              console.log(relevantDepts);
              setDoctor({
                ...allDoctorData, 
                allDept: relevantDepts, 
              });
            }
            else if(hospitalName){
              const relevantDepts = allDoctorData.allDept.filter(
                (dept: any) => dept?.dept?.hospital?.name.toLowerCase().trim() === hospitalName.toLowerCase().trim()
              );
  ...
  useEffect(() => {
    if (selectedDepartment && doctor) {
      const selectedDept = doctor.allDept.find(
        (dept: any) => dept.dept.name === selectedDepartment
      );
      // console.log(doctor.allDept);
      if (selectedDept) {
      //   setFees(selectedDept.consultationFee);
      console.log(selectedDept);
      setFees(selectedDept.consulatationFees);
      setDeptId(selectedDept.deptId);
      }
    }
              ...
              <div className="flex-1 space-y-16">
                <div className="text-center space-y-6 pt-4">
                  ...
                  <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Button
                      variant="outline"
                      onClick={() => router.push("/patient-auth")}
                    >
                      Login
                    </Button>
                    <Button
                  ...
                  <div className="text-muted-foreground font-medium text-red-500 pb-4">
                    Note : Patient is required to login to book an appointment.
                    Exception is made while using emergency services. Extra
                    charges may apply on using emergency services.
                  </div>
                </div>
              </div>
            </div>
      ...
      <Card className="w-full max-w-3xl shadow-xl rounded-2xl overflow-hidden">
        ...
        <CardContent className="space-y-6 p-6">
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Select onValueChange={handleDepartmentChange}>
                  ...
                  <SelectContent>
                    {doctor.allDept.map((dept: any) => (
                      <SelectItem key={dept.dept.name} value={dept.dept.name}>
                        {dept.dept.name} {!hospitalKey &&("("+dept.dept.hospital.name+")")}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <div className="bg-blue-50 p-5 rounded-lg">
            ...
            <p className="text-3xl font-extrabold text-blue-600">
              &#8377;{fees !== null ? fees.toFixed(2) : "Select a department"}
            </p>
          </div>
        </CardContent>
        <CardFooter className="p-6 bg-gray-50">
          <Button
            ...
            size="lg"
            disabled={!date || !timeSlot || !selectedDepartment}
